...
def main():
    ...
    # Asegurarse de que min_alt no sea menor que ninguna altitud real
min_alt_real = min([a for a in altitudes if a > 0])
max_alt_real = max(altitudes)
rango_alt = max_alt_real - min_alt_real

puntos_svg = []
for d_acum, a in zip(distancias_acum, altitudes):
    # limitar altitud dentro del rango
    a = max(a, min_alt_real)
    # mapeo Y invertido
    y = 25 + (max_alt_real - a) / rango_alt * (alto_svg - 50)
